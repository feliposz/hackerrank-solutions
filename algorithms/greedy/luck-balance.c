...
int luckBalance(int k, int contests_rows, int contests_columns, int** contests) {
    int *important = malloc(contests_rows * sizeof(int));
    int luck = 0;
    int count = 0;
    for (int i = 0; i < contests_rows; i++) {
        if (contests[i][1]) {
            // save important contests for sorting
            important[count++] = contests[i][0];
        } else {
            // lose uninmportant contest to grow luck
            luck += contests[i][0];
        }
    }
    // sort important contests by value
    qsort(important, count, sizeof(int), cmp);
    for (int i = 0; i < count; i++) {
        // lose the contests that will give more luck and win the others
        int sign = i < count - k ? -1 : 1;
        luck += sign * important[i];
    }
    return luck;
}
